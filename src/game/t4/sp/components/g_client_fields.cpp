...
namespace t4
{
    namespace sp
    {
        ...
        void GScr_AddFieldsForClient_Hook()
        {
            ...
            {
                Scr_AddClassField(0, fields_extended[i].name, static_cast<unsigned short>(i) | CLIENT_FIELD_MASK, SCRIPTINSTANCE_SERVER);
        ...
        void Scr_GetClientField(gclient_s *client, unsigned int offset)
        {
            ...
            if (field->getter)
                ...
                Scr_GetGenericField(reinterpret_cast<unsigned char *>(client), field->type, field->ofs, SCRIPTINSTANCE_SERVER, field->whichbits);
        }
