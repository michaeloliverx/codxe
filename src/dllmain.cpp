...
void monitor_title_id()
{
    ...
    for (;;)
    {
        ...
        else if (current_title_id == GAME_TITLE_ID_QOS)
        {
            if (strncmp((char *)0x82047EC4, "startSingleplayer", 17) == 0)
            {
                if (!in_xenia)
                    Sleep(1000);
                xbox::show_notification(L"CODxe - QOS Singleplayer Detected");
                qos::sp::init();
            }
            else if (strncmp((char *)0x8200236C, "multiplayer", 11) == 0)
            {
                if (!in_xenia)
                    Sleep(1000);
                xbox::show_notification(L"CODxe - QOS Multiplayer Detected");
                qos::mp::init();
            }
            else
            {
                xbox::show_notification(L"CODxe - QOS Unsupported Executable");
            }
            return;
        }
        else if (current_title_id == GAME_TITLE_ID_T5)
        {
            if (strncmp((char *)0x82018364, "startSingleplayer", 17) == 0)
            {
                if (!in_xenia)
                    Sleep(1000);
                xbox::show_notification(L"CODxe - T5 Singleplayer Detected");
                t5::sp::init();
            }
            else if (strncmp((char *)0x82001998, "multiplayer", 11) == 0)
            {
                if (!in_xenia)
                    Sleep(1000);
                xbox::show_notification(L"CODxe - T5 Multiplayer Detected");
                t5::mp::init();
            }
            else
            {
...
int DllMain(HANDLE hModule, DWORD reason, void *pReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        xbox::ExCreateThread(nullptr, 0, nullptr, nullptr, reinterpret_cast<PTHREAD_START_ROUTINE>(monitor_title_id),
                             nullptr, 2);
        break;
